...
if __name__ == '__main__':
    ...
    ft = Bintang("myft")
    filepath = r"C:\Users\60145210\Documents\AFMO Data\ems_repo\Pathology.xlsx"
    ...
    colmap = copyconf.ColMapConf("Local Asset ID","BmeNumber")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("Classification L4 (Required)","Name")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("Supplier Name (Required)","Supplier")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("Manufacturer Name","ManufacturerName")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("Model Number (if Available - if not N/A)","ModelNumber")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("Serial Number","SerialNumber")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("Position (if known)","LocationName")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("Property (Required)","Hospital")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("Purchase Cost (Required)","Price")
    c.colmap_list.append(colmap)
    colmap = copyconf.ColMapConf("In Service Date (Mandatory)","StartUpDate")
    c.colmap_list.append(colmap)
